...
| Extract remaining conditional reference chunks | 3 | 2 | M (2) | 4.0 | Idea | Continuation of the compaction→extraction pivot (prose compaction proven exhausted, <1% on these files). Pull conditional chunks into own files: the-midwife → `worldbuilding-mode.md` (~1.8KB), reconcile → world-evolution step (~1.7KB), session-prep → world-threads/narrative-plans (~1.5KB). Each loads only when its structure exists. check-procedures split + PC-body extraction already shipped (v1.8.7). May fall below effort line — measure per chunk. |
