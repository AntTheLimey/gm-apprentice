...
| Obsidian vault-picker | 3 | 2 | M (2) | 4.0 | Idea | Campaign config drives vault switching: on session start, pin the correct `vault="<name>"` for Obsidian CLI calls per campaign |
...
- ~~Obsidian MCP stack removal~~ — The archived MCP server stack (MCP Tools + Local REST API plugins, `search_vault` tool detection) removed from every skill, doc, and the README. `shared/filesystem-mode.md` rewritten as the Vault Access Reference: filesystem tools as the always-on default, the official Obsidian CLI (1.12.7+) as an optional tier for ranked search and graph queries with mandatory `vault="<name>"` pinning, once-per-session detection with runtime-failure fallback, and kepano/obsidian-skills as the canonical CLI reference. README gains CLI enablement steps and an MCP migration note. A/B benchmark (Canticle vault, 3 query pairs): identical result quality; grep cheaper for exact-term search (~20% fewer tokens); CLI 2.4× faster for vault-wide orphan detection. plugin v1.8.4 (PR #TBD)
