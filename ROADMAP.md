...
- ~~GURPS sheet current-encumbrance-row highlight from markdown tables~~ — The Encumbrance block's `.cur` outline was only reachable via a frontmatter `encumbrance:` array; plain markdown tables (the common case) hard-coded `current: false`. The parser now flags the current row from an explicit marker on the Level cell (trailing `*` canonical, `←` and `(current)` also accepted, stripped from display) or, when no row is marked, by matching `## Current Status`'s `**Enc:**` value against level names (case-insensitive, parentheticals ignored). Both table paths (standalone section and Equipment subsection) covered; at most one row flagged from any source; the status fallback also covers frontmatter arrays without a `current` flag, and sheets with neither marker nor matching status render unchanged. Verified against Dead End's Six.md — the status fallback puts `class="e1 cur"` on Light (1). tool 1.5.1, plugin v1.8.9 (PR #TBD)
