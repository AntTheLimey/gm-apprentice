...
**Reconcile-to-Prep handoff:** The `## Reconciliation Context`
section captures all reconciliation outputs so that if prep
continues in a new conversation, context is preserved without
re-reading the full vault. Steps 7-10 read this section and
only gather what's new.
...
**7. Recap** — Present the narrative recap from the last
session note as "Previously on..." (generated by
session-wrapup).
...
If Reconcile ran, steps 7-10 read `## Reconciliation Context`
and only gather what's new beyond what reconciliation already
established.

## Phase 2: Prep Forward — Creative Planning

References `skills/ttrpg-expert/arc-spotlight-reference.md`
(arc stages, touchpoint taxonomy, A/B/C structure, spotlight
thresholds) and the active system's `session-procedures.md`
(Arc Drivers section — how system mechanics fuel arcs).
...
Steps 7-10 are independent vault reads. Parallelizable via
sub-agents if the Agent tool is available. Sequential
otherwise.

Steps 11-14 are sequential creative planning. Each step
builds on the previous. Run in order on the primary model.

Steps 15-16 are independent quality checks. Parallelizable
via sub-agents if available.

If sub-agents are unavailable, run all steps sequentially.
